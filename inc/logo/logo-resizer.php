 ...
 */
function business_pro_douali_customize_logo_resize( $html ) {
	...
	if ( is_numeric( $size ) && is_numeric( $custom_logo_id ) ) {
		...
		if ( $logo['width'] >= $logo['height'] ) {
			$output = business_pro_douali_min_max( $logo['height'], $logo['width'], $max['height'], $max['width'], $size, $min );
			$img = array(
				'height'	=> $output['short'],
				'width'		=> $output['long']
			);
		// portrait
			...
			$output = business_pro_douali_min_max( $logo['width'], $logo['height'], $max['width'], $max['height'], $size, $min );
			$img = array(
				'height'	=> $output['long'],
				'width'		=> $output['short']
			);
...
add_filter( 'get_custom_logo', 'business_pro_douali_customize_logo_resize' );

/* Helper function to determine the max size of the logo */
function business_pro_douali_min_max( $short, $long, $short_max, $long_max, $percent, $min ){
	$ratio = ( $long / $short );
 ...
 */
function business_pro_douali_customize_controls_js() {
	wp_enqueue_script( 'business-pro-customizer-controls', esc_url(get_template_directory_uri()) . '/inc/logo/js/customize-controls.js', array( 'jquery', 'customize-preview' ), '201709071000', true );
}
add_action( 'customize_controls_enqueue_scripts', 'business_pro_douali_customize_controls_js' );

/**
 * Adds CSS to the Customizer controls.
 */
function business_pro_douali_customize_css() {
	wp_add_inline_style( 'customize-controls', '#customize-control-logo_size input[type=range] { width: 100%; }' );
}
add_action( 'customize_controls_enqueue_scripts', 'business_pro_douali_customize_css' );

/**
 * Testing function to remove logo_size theme mod
 */
function business_pro_douali_remove_theme_mod() {
	if ( isset( $_GET['remove_logo_size'] ) && 'true' == $_GET['remove_logo_size'] ){
		set_theme_mod( 'logo_size', '' );
	}
}
add_action( 'wp_loaded', 'business_pro_douali_remove_theme_mod' );
